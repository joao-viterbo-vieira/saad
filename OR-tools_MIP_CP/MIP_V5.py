...
def solve_capacitated_warehouse_location(
    ...
    amountServed = {
        (i, j): solver.IntVar(0, 15000, f"amountServed_{i}_{j}")
        for i in Warehouses
        for j in Customers
    }
    # ↑ Aumentei para 5.000.000 só para evitar limite muito apertado.
    ...
    if constraints_config.get("minimum_capacity_usage", False):
        # A versão do docplex não faz floor no 0.8 * capacity[i].
        # No OR-Tools, podemos escrever a restrição com multiplicação por float.
        # Mas lembre-se: 'amountServed' é int, logo teremos 1 ou 2 unidades de diferença
        # se capacity[i] * 0.8 for não-inteiro. Ainda assim, é mais fiel ao docplex do que floor.
        for i in Warehouses:
            #  No docplex: sum(amountServed[i, j]) >= 0.8*capacity[i]*x[i]
            #  Aqui, OK usar float; o solver CBC trata como "sum(...) >= float".
            solver.Add(
    ...
    # ==============================
    # 9. (NOVA) Amarrar amountServed a y (como no docplex)
    # ==============================
    # Docplex: amountServed[i,j] >= y[i,j]
    # Se y[i,j] = 1, tem de servir pelo menos 1 unidade.
    # Se preferir permitir 0.5, deveria mudar amountServed para contínuo.
    # Aqui, mantemos inteiro e exigimos >= 1 * y[i,j].
    # Se você quiser ativar sempre, basta remover o 'if ...:'
    if True:  # ou, se preferir, um config.get("tie_amountServed_to_y", True)
        for i in Warehouses:
            for j in Customers:
                solver.Add(amountServed[i, j] >= y[i, j])
    ...
    if status == pywraplp.Solver.OPTIMAL:
        ...
        for i in Warehouses:
            if x[i].solution_value() > 0.5:
                ...
                print(
                    f"  - Armazém {i + 1} está aberto. "
                    f"Quantidade Total Servida: {total_served:.2f} unidades"
                )
...
if __name__ == "__main__":
    ...
    data_file = "/Users/joaovieira/Desktop/Trabalho_SAAD/data/facility_location_124.dat"
